...
use school;
  ...
  INSERT INTO subject(name, School_id, maxcapacity) values ("Biology", 1, 6);
  INSERT INTO subject(name, School_id, maxcapacity) values ("Chemistry", 1, 3);
  INSERT INTO subject(name, School_id, maxcapacity) values ("Geometry", 1, 4);
  INSERT INTO subject(name, School_id, maxcapacity) values ("Art", 1, 2);
  INSERT INTO subject(name, School_id, maxcapacity) values ("Physics", 1, 5);
...
-- UPDATE student SET first_name = "Ale" WHERE id = 3;
-- UPDATE student SET first_name = "Gio" WHERE subject_id = 1;
-- UPDATE director SET email = "iaintsk@gmail.com" WHERE id = 1;
-- UPDATE director SET last_name = "cinaridze" WHERE id > 2 AND id < 4;
-- UPDATE teacher SET first_name = "dato" WHERE last_name = "iremadze";
-- UPDATE student SET subject_id = 3 WHERE name like "rel%";
-- UPDATE teacher SET phone_number = 1826352 WHERE Date_Of_Birth LIKE '2001%';
-- UPDATE student SET first_name ="davit" WHERE Date_Of_Birth LIKE '2003%';
-- UPDATE student SET first_name = "dato" WHERE Date_Of_Birth LIKE '2004%';
...
-- DELETE FROM student WHERE phone_number = 98363847;
...
-- DELETE FROM student WHERE Subject_id > 1;

--  SELECT student.first_name, subject.name FROM student LEFT join subject ON student.subject_id=subject.id
--  SELECT subject.name, student.first_name FROM student RIGHT join subject ON student.subject_id=subject.id
 -- SELECT subject.name, student.first_name FROM student INNER join subject ON student.subject_id=subject.id
--   SELECT student.first_name, subject.name FROM student
